...
void Character::update(float dt)
{
    if (isDead) {
        deathAnim.update(dt);
        deathAnim.applyToSprite(sprite);
        if (deathAnim.isFinished()) {
            sprite.setColor(sf::Color(255, 255, 255, 0)); // ẩn sprite
        }
        return;
    }
    ...
    if (isHurt) {
        hurtTimer -= dt;
        hurtAnim.update(dt);
        hurtAnim.applyToSprite(sprite);
        sprite.setColor(sf::Color(255, 100, 100)); // Nhay do
    ...
    if (isAttacking)
    {
        ...
        attackAnim.applyToSprite(sprite);

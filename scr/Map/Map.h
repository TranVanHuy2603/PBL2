...
{
private:
    int height, width; //cieu cao va chieu rong cua ban do
    std::vector<std::vector<int>> grid;//ma tran chua thong tin cac diem dung cho A*
public:
    Map(int, int);

    int get_height(); //lay chieu cao
    int get_width(); //lay chieu rong
    std::vector<std::vector<int>> get_grid(); //lay ma tran

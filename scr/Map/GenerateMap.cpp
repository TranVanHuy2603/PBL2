...
// Hàm fade và grad của Perlin Noise
...
float perlin(float x, float y) {
    ...
    return (lerp(x1, x2, v) + 1) / 2; // chuẩn hóa về [0,1]
}
...
int n_main() {
    ...
    float scale = 0.01f; // scale nhỏ = map mượt

    for (int i = 0; i < MAP_SIZE; i++) {
        for (int j = 0; j < MAP_SIZE; j++) {

            float h = perlin(i * scale, j * scale);

            int type;
            if (h < 0.25) type = 4;         // Water
            else if (h < 0.40) type = 7;    // Sand
            else if (h < 0.55) type = 0;    // Grass
            else if (h < 0.70) type = 1;    // Forest
            else if (h < 0.85) type = 6;    // Stone / Hills
            else type = 10 + rand() % 3;    // Rare ores
    ...
    outFile.close();
    std::cout << "✅ Map đã sinh vào thư mục assets/map/mapdata.txt\n";
    return 0;
}
